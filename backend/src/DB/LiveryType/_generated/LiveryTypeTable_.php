<?php
/**
 * This file was generated by Atlas. Changes will be overwritten.
 */
declare(strict_types=1);

namespace LiveryManager\DB\LiveryType\_generated;

use Atlas\Table\Table;
use LiveryManager\DB\LiveryType\LiveryTypeRow;
use LiveryManager\DB\LiveryType\LiveryTypeTableSelect;

/**
 * @method ?LiveryTypeRow fetchRow(mixed $primaryVal)
 * @method LiveryTypeRow[] fetchRows(array $primaryVals)
 * @method LiveryTypeTableSelect select(array $whereEquals = [])
 * @method LiveryTypeRow newRow(array $cols = [])
 * @method LiveryTypeRow newSelectedRow(array $cols)
 */
abstract class LiveryTypeTable_ extends Table
{
    public const DRIVER = 'mysql';

    public const NAME = 'livery_type';

    public const COLUMNS = [
  'id' => 
  [
    'name' => 'id',
    'type' => 'int unsigned',
    'size' => 10,
    'scale' => 0,
    'notnull' => true,
    'default' => NULL,
    'autoinc' => false,
    'primary' => true,
    'options' => NULL,
  ),
  'name' => 
  [
    'name' => 'name',
    'type' => 'varchar',
    'size' => 50,
    'scale' => NULL,
    'notnull' => true,
    'default' => '',
    'autoinc' => false,
    'primary' => false,
    'options' => NULL,
  ),
  'created_at' => 
  [
    'name' => 'created_at',
    'type' => 'datetime',
    'size' => NULL,
    'scale' => NULL,
    'notnull' => true,
    'default' => 'current_timestamp()',
    'autoinc' => false,
    'primary' => false,
    'options' => NULL,
  ),
);

    public const COLUMN_NAMES = [
        'id',
        'name',
        'created_at',
    ];

    public const PRIMARY_KEY = [
        'id',
    ];

    public const COMPOSITE_KEY = false;

    public const AUTOINC_COLUMN = null;

    public const AUTOINC_SEQUENCE = null;

    public const ROW_CLASS = LiveryTypeRow::CLASS;
}
